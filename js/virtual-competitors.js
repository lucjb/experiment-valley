function normalizeDecisionForStatus(decision, experiment) {
    const statusInfo = experiment?.status || {};
    const stoppedValue = (window.EXPERIMENT_STATUS && window.EXPERIMENT_STATUS.STOPPED) || 'STOPPED';
    if (statusInfo.state === stoppedValue && decision === EXPERIMENT_DECISION.KEEP_RUNNING) {
        const resetValue = (window.EXPERIMENT_DECISION && window.EXPERIMENT_DECISION.RESET_EXPERIMENT) || EXPERIMENT_DECISION.RESET_EXPERIMENT || 'RESET_EXPERIMENT';
        return resetValue;
    }
    return decision;
}
    ...
    HiPPO: {
        ...
        makeDecision: (experiment) => {
            ...
            return {
                ...
                decision: normalizeDecisionForStatus(decision, experiment),
                followUp: followUp
            };
        }
    },
